...
const statusBadge: Record<string, { c: string; label: string }> = {
  ...
  completed: { c: "border-[#2563EB]/30 bg-[#2563EB]/10 text-[#2563EB]", label: "Completed" },
...
function BoostPage() {
  ...
  return (
    <div className="min-h-screen bg-white px-4 pb-28 pt-6 font-sans text-slate-900 dark:bg-[#0F1332] dark:text-slate-100">
      <div className="mx-auto max-w-md">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-slate-900 dark:text-white">Boost Account</h1>
        </div>
        ...
        <div className="mb-5 flex items-center justify-between overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800/80 dark:bg-slate-900/90">
          ...
          <span className="text-base font-black text-[#2563EB]">{naira(balance)}</span>
        </div>
        ...
        <form onSubmit={(e) => void handleOrder(e)} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-sm text-slate-900 placeholder-slate-400 outline-none transition focus:border-[#2563EB] dark:border-slate-800 dark:bg-slate-900/90 dark:text-white dark:placeholder-slate-600"
            />
          ...
          <div className="space-y-2 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/80">
            ...
            {qty > 0 && selected && (
              <div className="flex items-center justify-between border-t border-slate-800 pt-2">
                <span className="text-xs font-semibold text-[#2563EB]">Estimated Total</span>
                <span className="text-base font-black text-[#2563EB]">{naira(total)}</span>
              </div>
            )}
          ...
          <button
            ...
            className="flex w-full items-center justify-center space-x-2 rounded-2xl bg-[#2563EB] py-4 text-sm font-bold tracking-wide text-white shadow-lg shadow-[#2563EB]/25 transition hover:bg-blue-700 disabled:opacity-50"
          >
        ...
        <section className="mt-8 space-y-3">
          <div className="flex items-center justify-between px-1">
            <h2 className="text-sm font-bold text-slate-900 dark:text-white">My Orders</h2>
                  ...
                  >
                    ...
                    {o.target_url && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="mt-2 flex items-center gap-1.5 truncate text-[12px] font-medium text-[#2563EB]"
                      >
        ...
        >
          {filteredPlatforms.map((p) => (
            ...
            >
              ...
              {platform === p && <CheckCircle2 size={16} className="text-[#2563EB]" />}
            </button>
          ))}
        </SelectModal>
      )}
            ...
            filteredCategories.map((c) => (
              ...
              >
                <div className="flex items-center space-x-2.5">
                  <Folder
                    size={16}
                    className={category === c ? "text-[#2563EB]" : "text-slate-500"}
                  />
                  <span className="text-sm">{c}</span>
                </div>
                {category === c && <CheckCircle2 size={16} className="text-[#2563EB]" />}
              </button>
            ))
          )}
        </SelectModal>
              ...
              return (
                <button
                  ...
                  className={`mb-2 flex w-full flex-col rounded-xl border p-3.5 text-left transition ${
                    active
                      ? "border-[#2563EB]/40 bg-[#2563EB]/10 text-slate-900 dark:text-white"
                      : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50 dark:border-slate-800/80 dark:bg-slate-950/40 dark:text-slate-200 dark:hover:bg-slate-800/50"
                  }`}
                >
                  <span className="mb-1 text-xs font-bold text-slate-900 dark:text-white">{s.service_type}</span>
      ...
      {showFund && selected && (
        <div className="fixed inset-0 z-50 grid place-items-end bg-black/70 p-4 sm:place-items-center">
          <div className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <h3 className="mt-3 text-lg font-black text-slate-900 dark:text-white">Insufficient balance</h3>
            ...
            <div className="mt-4 flex gap-2">
              <button
                ...
                className="flex-1 rounded-xl border border-slate-200 bg-slate-100 py-3 text-sm font-bold text-slate-800 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
              >
                Cancel
              </button>
              <Link
                to="/fund"
                className="flex-1 rounded-xl bg-[#2563EB] py-3 text-center text-sm font-black text-white"
              >
                Fund wallet
              </Link>
            </div>
      ...
      <div className="flex max-h-[85vh] w-full max-w-md flex-col overflow-hidden rounded-t-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-800 dark:bg-slate-900 sm:rounded-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4 dark:border-slate-800">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:text-slate-900 dark:bg-slate-800 dark:text-slate-400 dark:hover:text-white"
          >
            <X size={16} />
          </button>
        </div>
        <div className="py-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search..."
              className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-4 text-xs text-slate-900 placeholder-slate-400 outline-none focus:border-[#2563EB] dark:border-slate-800 dark:bg-slate-950 dark:text-white dark:placeholder-slate-600"
            />
          </div>
        </div>
