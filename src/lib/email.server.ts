/**
 * Transactional email for Verxor.
 * Prefer RESEND_API_KEY (works on Cloudflare + Railway).
 * Fallback: SMTP_* env (Zoho) when running on Node (Railway).
 *
 * From: Verxor <official@verxor.com>
 */

const FROM = process.env.EMAIL_FROM || "Verxor <official@verxor.com>";
const APP_URL = (process.env.APP_URL || process.env.VITE_APP_URL || "https://verxor.com").replace(
  /\/$/,
  "",
);
...
export async function sendPinResetEmail(to: string, token: string) {
  ...
  const subject = "Reset your Verxor PIN";
  const text = `Reset your Verxor 4-digit PIN:\n\n${link}\n\nThis link expires in 30 minutes. If you did not request this, ignore this email.`;
  const html = `
  <div style="font-family:Inter,system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#0f172a">
    ...
    <p style="margin:0 0 16px;line-height:1.5;color:#475569">
      We received a request to reset your Verxor 4-digit transaction PIN.
...
export async function sendPinChangedEmail(to: string) {
  const subject = "Your Verxor PIN was changed";
  const text =
    "Your Verxor 4-digit PIN was changed successfully. If this was not you, contact support immediately and secure your account.";
  const html = `
  <div style="font-family:Inter,system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#0f172a">
    <h1 style="font-size:20px;margin:0 0 12px">PIN changed</h1>
    <p style="margin:0 0 16px;line-height:1.5;color:#475569">
      Your Verxor 4-digit transaction PIN was changed successfully.
    </p>
...
export async function sendWelcomeEmail(to: string, firstName?: string) {
  ...
  const subject = "Welcome to Verxor";
  const text = `Hi ${name}, welcome to Verxor. Your wallet, virtual numbers and boosts are ready.`;
  const html = `
  <div style="font-family:Inter,system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#0f172a">
    <h1 style="font-size:20px;margin:0 0 12px">Welcome to Verxor 👋</h1>
    <p style="margin:0 0 16px;line-height:1.5;color:#475569">
