...
> {
  ...
  return cached(`didww:avail:${iso}:${limit}`, 3 * 60 * 1000, async () => {
    try {
      ...
      const availRes = await fetchWithTimeout(availUrl, { method: "GET", headers: headers() }, 4000);
